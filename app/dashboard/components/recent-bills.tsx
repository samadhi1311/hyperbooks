					...
					memoizedData.slice(0, 5).map((doc, index) => (
						<div className='flex items-center gap-4 md:items-start' key={index}>
							...
							<div className='grid gap-0.5'>
								<span className='inline-block text-sm font-medium xl:hidden'>{userData?.currency + ' ' + doc.amount.toFixed(2)}</span>
							...
							<span className='ml-auto hidden font-medium xl:inline-block'>{userData?.currency + ' ' + doc.amount.toFixed(2)}</span>
						</div>
					))
				) : (
					<div className='mb-4'>
